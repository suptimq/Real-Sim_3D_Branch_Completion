...
from losses.losses import RepulsionLoss, CPCLoss, compute_loss_pre, compute_loss, compute_loss_supervision
...
class PCTransformer(nn.Module):
    def __init__(self, config):
        ...
        skel_decoder_config = config.skel_decoder_config
        ...
        self.skel_query = nn.Sequential(
            nn.Linear(global_feature_dim + 3, 1024),
            nn.GELU(),
            nn.Linear(1024, 1024),
            nn.GELU(),
            nn.Linear(1024, skel_decoder_config.embed_dim)
        )
        ...
        if skel_decoder_config.embed_dim == encoder_config.embed_dim:
            self.mem_link2 = nn.Identity()
        else:
            self.mem_link2 = nn.Linear(encoder_config.embed_dim, skel_decoder_config.embed_dim)

        # Coarse Level 2 : Decoder
        self.decoder = PointTransformerDecoderEntry(decoder_config)
        self.skel_decoder = PointTransformerDecoderEntry(skel_decoder_config)
    ...
    def forward(self, xyz):
        ...
        if self.training:
            ...
            skel_q = self.skel_decoder(q=skel_q, v=mem2, q_pos=coarse, v_pos=coor, denoise_length=denoise_length)
            
            return q, skel_q, coarse, denoise_length
            ...
            skel_q = self.skel_decoder(q=skel_q, v=mem2, q_pos=coarse, v_pos=coor)

            return q, skel_q, coarse, 0
...
class AdaPoinTr(nn.Module):
    def __init__(self, config, **kwargs):
        super().__init__()
        self.trans_dim = config.model.decoder_config.embed_dim
        self.skel_trans_dim = config.model.skel_decoder_config.embed_dim
        self.num_query = config.model.num_query
        self.num_points = getattr(config.model, 'num_points', None)
        ...
        self.fold_step = 8
        self.base_model = PCTransformer(config.model)
        ...
        # Skeleton Net
        # -------------------------
        self.num_skel_points = config.model.num_skel_points
        self.cvx_weights_mlp = nn.Sequential(
            nn.Dropout(0.2),
            nn.Conv1d(self.skel_trans_dim+1027, 1024, 1),
            nn.BatchNorm1d(1024),
            nn.ReLU(True),
            ...
            nn.Conv1d(128, self.num_skel_points, 1),
            nn.BatchNorm1d(self.num_skel_points),
            nn.Softmax(dim=2),
        )

        self.repulsion_loss_func = None
        self.cpc_loss_func = None
        self.cpc_loss_ep = config.loss.additional_metrics.cpc.turn_on_ep if 'cpc' in config.loss.additional_metrics.keys() else config.max_epoch+1
        self.skel_pre_compute_epoch = config.loss.additional_metrics.skel.pre_compute_epoch
        self.skel_supervised = config.loss.additional_metrics.skel.supervised
    ...
    def build_loss_func(self, loss):
        ...
        if 'cpc' in loss.additional_metrics.keys():
            self.cpc_loss_func = CPCLoss(alpha=loss.additional_metrics.cpc.alpha)
    ...
    def get_loss(self, ret, gt, **kwargs):
        ...
        pred_coarse, denoised_coarse, denoised_fine, pred_fine, skel_xyz, skel_r = ret
        ...
        loss_dict['coarse_loss'] = loss_denoised
        loss_dict['fine_loss'] = loss_recon

        if self.repulsion_loss_func is not None:
            loss_dict['repulsion_loss'] = self.repulsion_loss_func(pred_coarse)

        epoch = kwargs.get('epoch', 0)
        if self.cpc_loss_func is not None:

            if epoch > self.cpc_loss_ep:
                ...
                p2p_var1, p2s_mean1 = self.cpc_loss_func(pred_coarse, centers, center_radii)
                p2p_var2, p2s_mean2 = self.cpc_loss_func(pred_fine, centers, center_radii)

                loss_dict['cpc_loss_p2p_var'] = p2p_var1 + p2p_var2
                loss_dict['cpc_loss_p2s_mean'] = p2s_mean1 + p2s_mean2

        if self.skel_supervised:
            centers = kwargs.get('centers', None)
            center_radii = kwargs.get('center_radii', None)
            assert centers is not None and center_radii is not None, 'Missing Segment GT Data'
            loss_skel, loss_skelr = compute_loss_supervision(centers, center_radii, skel_xyz, skel_r)
            loss_dict['skel_loss'] = loss_skel + loss_skelr
        elif epoch < self.skel_pre_compute_epoch:
            loss_dict['skel_loss'] = compute_loss_pre(gt, skel_xyz)
        else:
            loss_dict['skel_loss'] = compute_loss(gt, skel_xyz, skel_r, w1=0.3, w2=0.4)

        return loss_dict

    def forward(self, xyz):
        q, skel_q, coarse_point_cloud, denoise_length = self.base_model(xyz) # B M C and B M 3
        ...
        skel_rebuild_feature = torch.cat([
            global_feature.unsqueeze(-2).expand(-1, M, -1),
            skel_q,
            coarse_point_cloud], dim=-1)  # B M 1027 + C
        weights = self.cvx_weights_mlp(skel_rebuild_feature.transpose(1, 2))  # B num_skel M
        skel_xyz = torch.sum(weights[:, :, :, None] * coarse_point_cloud[:, None, :, :], dim=2)  # B num_skel M 1 * B 1 M 3
        min_dists, _ = loss_utils.closest_distance_with_batch(coarse_point_cloud, skel_xyz, is_sum=False)
        ...
        if self.training:
            ...
            return pred_coarse, denoised_coarse, denoised_fine, pred_fine, skel_xyz, skel_r
            ...
            return coarse_point_cloud, rebuild_points, skel_xyz, skel_r