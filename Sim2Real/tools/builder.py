...
def resume_model(base_model, discriminator, args, logger = None):
    ...
    assert state_dict.get('discriminator') is not None, '[RESUME INFO] not found discriminator weights!'
    d_ckpt = {k.replace("module.", ""): v for k, v in state_dict['discriminator'].items()}
    print_log(f'[RESUME INFO] load discriminator!')
...
def resume_optimizer(optimizer, optimizer_D, args, logger = None):
    ckpt_path = os.path.join(args.experiment_path, 'ckpt-last.pth')
    if not os.path.exists(ckpt_path):
        print_log(f'[RESUME INFO] no checkpoint file from path {ckpt_path}...', logger = logger)
        return 0, 0, 0
    ...
    if state_dict.get('optimizer_D') is not None:
        optimizer_D.load_state_dict(state_dict['optimizer_D'])
        print_log(f'[RESUME INFO] load optimizer_D parameters!')
    else:
        print_log(f'[RESUME INFO] not found optimizer_D parameters!')

def save_checkpoint(base_model, optimizer, discriminator, optimizer_D, epoch, metrics, best_metrics, prefix, args, logger = None):
    if args.local_rank == 0:
        torch.save({
                    'base_model' : base_model.module.state_dict() if args.distributed else base_model.state_dict(),
                    'optimizer' : optimizer.state_dict(),
                    'discriminator': discriminator.module.state_dict() if args.distributed else discriminator.state_dict(),
...
def load_model(base_model, discriminator, ckpt_path, logger = None):
    if not os.path.exists(ckpt_path):
    ...
    if base_ckpt.keys() == base_state_dict.keys():
        base_model.load_state_dict(base_ckpt)
    else:
        print_log(f'base_model is not identical to the ckpt weights! Only load matched keys', logger = logger)
        # filter the old state dictionary to match keys with the new model
        base_ckpt = {k: v for k, v in state_dict.items() if k in base_state_dict}
        # update the new state dictionary with the filtered old state dictionary
        base_state_dict.update(base_ckpt)
        # load the updated state dictionary into the new model
        base_model.load_state_dict(base_state_dict)

    if state_dict.get('discriminator') is not None:
        d_ckpt = {k.replace("module.", ""): v for k, v in state_dict['discriminator'].items()}
        print_log(f'load discriminator!')
        discriminator.load_state_dict(d_ckpt)
    else:
        print_log(f'not found discriminator weights!')
